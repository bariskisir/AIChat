...
export function renderMessages(refs: Refs, session: ChatSession, scrollOptions: MessageScrollOptions): void {
...
export function renderAssistantMessage(refs: Refs, model: UiModel, sessionId: string, messageId: string, text: string): void {
  ...
  message.text += text;
  const node = refs.chatMessages.querySelector<HTMLElement>(`[${"data-message-id"}="${cssEscape(messageId)}"] .${Constants.CSS.CH_BUBBLE_TEXT}`);
  if (node) {
